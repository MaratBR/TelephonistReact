...
import { Alert, Button, Card, Centered, Heading, Input, Screen, Stack } from "~src/components";
import TimeCountdown from "~src/components/TimeCountdown";
import { useNextParam, useProperPasswordState } from "~src/hooks";
...
export default function(props: {}) {
    ...
    if (auth.passwordResetExpiresAt < Date.now()) {
        ...
        return <Navigate to={{pathname: "/login", search: "?from=pwdreset.token_expired" + (next ? ("&next=" + next) : "")}} />
    }
        ...
        <Centered>
            <Card>
                ...
                <form onSubmit={onSubmit}>
                    <Stack spacing="md">
                        <Input variant="flushed" type="password" placeholder={t("new_password")} value={pwd.value} onChange={e => pwd.setValue(e.target.value)} />
                        <Input variant="flushed" type="password" placeholder={t("repeat_new_password")} value={pwd2} onChange={e => setPwd2(e.target.value)} />
                        {
                            pwd2 !== pwd.value ? <Alert color="danger">

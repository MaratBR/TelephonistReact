...
export class Api {
  ...
  createApplication(r: requests.CreateApplication): Promise<models.IdObject> {
    return this._client.post("user-api/applications", r).then((r) => r.data);
  }

  checkIfApplicationNameTaken(name: string): Promise<boolean> {
    return this._client
      .get("user-api/applications/check-if-name-taken", { params: { name } })
      .then((r) => r.data);
  }
  ...
  getApplicationTask(appID: string, taskID: string) {
    return this._client
      .get("user-api/applications/" + appID + "/defined-tasks/" + appID)
      .then((r) => r.data);
  }
  ...
  updateApplicationTask(
    ...
    return this._client
      .patch("user-api/applications/" + appID + "/defined-tasks/" + taskID)
      .then((r) => r.data);
  }
